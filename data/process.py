...
class PreData(object):
    ...
    def _get_we(self):
        ...
        pretrained_vocab_keys = list(w2v.key_to_index.keys())
        pretrained_vocab = set(pretrained_vocab_keys)
        print('making we...')
        ...
        for word, idx in self.word2i.items():
            if word in pretrained_vocab:
                ...
                n += 1
        self.we = ms.Tensor(we)
        if self.corpus_splitting == 1:
            with open('./processed/lin/we.pkl', 'wb') as file:
                pickle.dump(self.we, file)
        elif self.corpus_splitting == 2:
            with open('./processed/ji/we.pkl', 'wb') as file:
                pickle.dump(self.we, file)
        elif self.corpus_splitting == 3:
            with open('./processed/l/we.pkl', 'wb') as file:
                pickle.dump(self.we, file)
    ...
    def _make_char_table(self):
        ...
        lookup_table = ms.Tensor(lookup_table)
        len_table = ms.Tensor(len_table)
        if self.corpus_splitting == 1:
            with open('./processed/lin/char_table.pkl', 'wb') as file:
                pickle.dump([lookup_table, len_table], file)
        elif self.corpus_splitting == 2:
            with open('./processed/ji/char_table.pkl', 'wb') as file:
                pickle.dump([lookup_table, len_table], file)
        elif self.corpus_splitting == 3:
            with open('./processed/l/char_table.pkl', 'wb') as file:
                pickle.dump([lookup_table, len_table], file)
    ...
    def _make_subword_table(self):
        ...
        len_table = ms.Tensor(len_table)
        if self.corpus_splitting == 1:
            with open('./processed/lin/sub_table.pkl', 'wb') as file:
                pickle.dump([lookup_table, len_table], file)
        elif self.corpus_splitting == 2:
            with open('./processed/ji/sub_table.pkl', 'wb') as file:
                pickle.dump([lookup_table, len_table], file)
        elif self.corpus_splitting == 3:
            with open('./processed/l/sub_table.pkl', 'wb') as file:
                pickle.dump([lookup_table, len_table], file)
        print('{}/{} words found'.format(n, self.v_size))

    def _text2i(self, texts):
        l = len(texts)
        tensor = ms.Tensor(np.zeros((l, conf.max_sent_len)), ms.int64)
        for i in range(l):
            s = texts[i] + ['</s>']
    ...
    def _sense2i(self, senses):
        if self.corpus_splitting == 1 or self.corpus_splitting == 2:
            ...
            senseidx = 1
        l = len(senses)
        tensor = ms.Tensor(np.zeros((l,)), ms.int64)
        for i in range(l):
    ...
    def _pre_data(self):
        ...
        if self.corpus_splitting == 1:
            ...
            pre = './processed/l/'
        with open(pre + 'train.pkl', 'wb') as file:
            pickle.dump(train_data, file)
        with open(pre + 'dev.pkl', 'wb') as file:
            pickle.dump(dev_data, file)
        with open(pre + 'test.pkl', 'wb') as file:
...
def testpredata(corpus_splitting):
    ...
    with open(pre + 'char_table.pkl', 'rb') as file:
        lookup_table, len_table = pickle.load(file)
    with open(pre + 'sub_table.pkl', 'rb') as file:
        sub_lookup, sub_len = pickle.load(file)
    with open(pre + 'train.pkl', 'rb') as file:
        train_data = pickle.load(file)
    with open(pre + 'dev.pkl', 'rb') as file:
        dev_data = pickle.load(file)
    with open(pre + 'test.pkl', 'rb') as file:
